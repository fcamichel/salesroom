...
def get_date_or_error(date_id, user):
    """
    Tries to fetch a room (date) for the user, checking permissions along the way.
    """
    ...
    # Check permissions
    if not user.is_staff or user.groups.filter(name="Staff").exists():
